...
class LLMStrategyAgent:
    ...
    def __init__(self, 
        ...
        # Determine if we can use real LLM
        can_use_real_llm = (
        ...
        if can_use_real_llm:
            ...
            self.use_mock = False
            print(f"✅ Real LLM enabled: {model}")
        else:
            self.use_mock = True
            if not OPENAI_AVAILABLE:
                ...
                print("⚠️  Mock mode enabled - set use_mock=False for real LLM")
    ...
    def generate_strategy(self, situation_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            print(f"🤖 Calling real LLM ({self.model}) for strategy generation...")
