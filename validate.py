...
try:
    ...
    # Test a known location (Grand Canyon: 36.1°N, 112.1°W)
    params = {
        "x": -112.1,
        "y": 36.1,
        "units": "Meters",
...
try:
    ...
    # Test with a common location (San Francisco: 37.7749°N, 122.4194°W)
...
try:
    ...
    # Use small grid for fast testing, and disable strict mode for this test
    # to avoid failing on USGS API rate limits
    import real_scenario_builder
    original_strict = real_scenario_builder.REAL_DATA_STRICT
    real_scenario_builder.REAL_DATA_STRICT = False  # Allow soft failures for testing
    ...
    if len(scenarios) > 0:
        s = scenarios[0]
        print(f"✅ Step 5 passed: Successfully built scenario")
        ...
        print(f"   Slope: {s.terrain_slope.mean():.1f}° avg")
...
print("\nNext steps:")
print("  1. Build full dataset: python real_scenario_builder.py")
print("  2. Fine-tune Qwen: python train_qwen_wildfire.py")
print("  3. Train hybrid PPO+Qwen: python train_hybrid_real.py")
print("\nSee REAL_DATA_STRICT_MODE.md for details.\n")
